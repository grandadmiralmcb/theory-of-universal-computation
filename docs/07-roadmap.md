...
8. **One currency (CC) — DEVELOPED to a decision point (`docs/21-stratified-variant.md`).** The stratified variant is built out: lex cost = constraint-then-cost (exact rules on top, forces below); TS1/TS2 prove phenomenology forces the *dynamical floor* to be one-currency (splitting CI4 degenerates Newton; D-on-top freezes decoherence and loses T11); TS3 shows stratification is detectable only as exact rules; TS4+ST1 recover SB1-typing as a top stratum, with graceful forced-violation behavior brute typing cannot express. Synthesis candidate **CC′**: exact strata above one Archimedean floor — both RM2 branches realized in different roles. **Adoption of CC′+ST1 (closing the contention, at the price of one identification postulate) awaits owner adjudication.** CP's status remains the analogous weaker question.
